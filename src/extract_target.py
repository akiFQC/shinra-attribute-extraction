...
parser.add_argument('--query', '-o', default='/home/afukuchi/Codes/shinra-attribute-extraction/test_for_leaderboard/target_data/plain')
...
parser.add_argument('--output', default='/home/afukuchi/Codes/shinra-attribute-extraction/JP-5/for_leaderboard')
...
for category in ['City', 'Company']:
    ...
    tokens_query = glob.glob(os.path.join(args.query, category, '*.txt'))
    os.makedirs(token_dir_out, exist_ok=True)
    for p in tokens_query:
        page_id =  pathlib.Path(p).stem
